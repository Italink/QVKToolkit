...
void QFpsCamera::setup(QWindow* window)
{
	if (window) {
		window_ = window;
...
QMatrix4x4 QFpsCamera::getMatrixVP() const
{
...
bool QFpsCamera::eventFilter(QObject* watched, QEvent* event) {
	if (watched != nullptr && watched == window_) {
		...
		{
		case QEvent::Paint: {
			QCursor::setPos(window_->mapToGlobal(window_->geometry().center()));
			...
			QCursor::setPos(window_->mapToGlobal(window_->geometry().center()));  //将鼠标移动窗口中央
			break;
		}
		case QEvent::Resize: {
			projection_.setToIdentity();
